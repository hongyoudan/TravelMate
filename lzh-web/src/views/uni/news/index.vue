<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="要闻名称" prop="name">
        <el-input
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入要闻名称" />
        </el-form-item>
...
<script>
import { listNews, getNews, delNews, addNews, updateNews } from "@/api/uni/news";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [
          { required: true, message: "要闻名称不能为空", trigger: "blur" }
        ],
        content: [
          { required: true, message: "要闻内容不能为空", trigger: "blur" }
        ],
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除要闻编号为"' + ids + '"的数据项？').then(function() {
        return delNews(ids);
      }).then(() => {
        this.getList();
